...
internal class FakePokemonRemoteDataSource : PokemonRemoteDataSource {
    ...
    override suspend fun getPokemonList(page: Int, limit: Int): RpPokemonList {
        ...
        return RpPokemonList(
            count = 2,  // 예시로 2개의 포켓몬 반환
            next = null,
            previous = null,
            results = listOf(
                RpPokemonList.Pokemon(count = 1, name = "피카츄", url = "url1"),
                RpPokemonList.Pokemon(count = 2, name = "이상해씨", url = "url2")
            )
        )
    }

    override suspend fun getPokemonInfo(id: Int): RpPokemonInfo {
        // id에 따른 가짜 포켓몬 정보를 반환합니다.
        return when (id) {
            1 -> RpPokemonInfo(
                id = 1,
                name = "피카츄",
